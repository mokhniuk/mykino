 ...
 * Supports both provider-specific keys (OPENAI_API_KEY etc.) and the
 * community docker format (AI_PROVIDER + AI_API_KEY). */
export function detectProvider(): AIProvider {
  if (process.env.ANTHROPIC_API_KEY) return 'anthropic';
  if (process.env.OPENAI_API_KEY)    return 'openai';
  if (process.env.GEMINI_API_KEY)    return 'gemini';
  if (process.env.MISTRAL_API_KEY)   return 'mistral';
  if (process.env.OLLAMA_BASE_URL || process.env.OLLAMA_URL) return 'ollama';
  // Community docker format: AI_PROVIDER + AI_API_KEY
  const p = (process.env.AI_PROVIDER || '').toLowerCase() as AIProvider;
  if (p === 'ollama') return 'ollama';
  if (['anthropic', 'openai', 'gemini', 'mistral'].includes(p) && process.env.AI_API_KEY) return p;
  throw new Error('No AI provider configured. Set one of: ANTHROPIC_API_KEY, OPENAI_API_KEY, GEMINI_API_KEY, MISTRAL_API_KEY, OLLAMA_BASE_URL — or set AI_PROVIDER + AI_API_KEY');
}
...
async function callMistral(prompt: string, model?: string): Promise<string> {
  const key = process.env.MISTRAL_API_KEY || process.env.AI_API_KEY;
